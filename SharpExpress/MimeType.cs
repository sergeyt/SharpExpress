...
namespace SharpExpress
{
	...
	{
		// https://github.com/broofa/node-mime/blob/master/types/mime.types without comments
...
";
		...
		{
			Map = (
				from l in Regex.Split(Types, @"\r?\n").Where(s => !string.IsNullOrEmpty(s))
				let vals = Regex.Split(l, @"\s").Where(s => !string.IsNullOrEmpty(s)).ToArray()
				let type = vals[0]
				from ext in vals.Skip(1)
				select new KeyValuePair<string, string>(ext, type)
		...
		public static string ByExtension(string ext)
		{
			...
			string type;
			return Map.TryGetValue(ext[0] == '.' ? ext.Substring(1) : ext, out type) ? type : null;
		}
	}
}
